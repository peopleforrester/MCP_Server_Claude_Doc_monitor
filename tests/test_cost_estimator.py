...
async def test_estimate_cost_returns_all_fields(
    ...
    estimate = await estimate_cost(
        claims, docs, model="claude-sonnet-5", client=mock_client
    )
...
async def test_estimate_cost_empty_claims_returns_zero(docs: list[DocSection]) -> None:
    ...
    estimate = await estimate_cost([], docs, model="claude-sonnet-5", client=mock_client)
...
def test_model_pricing_known_models_have_rates() -> None:
    """Every model ID the project might use must be priced, current and legacy."""
    for model in (
        "claude-fable-5-1",
        "claude-opus-5",
        "claude-sonnet-5",
...
def test_model_pricing_sonnet_5_rates() -> None:
    """Sonnet 5 pricing per the published rate card (verified 2026-09-17)."""
    p = model_pricing("claude-sonnet-5")
